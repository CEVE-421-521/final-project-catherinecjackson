...
## this should be the annual maximujm surge height.  go to the noaa website and pull the annuals.  remember that if you pull the 1hr maximum, it is averaged over that 1 hour.  try to get the smalles time interval -- 6 min?
...
using CairoMakie  # Ensuring CairoMakie is included if not already

# Setup parameter names and mapping for the chain visualization
...
# Use the correct 'data' function from AlgebraOfGraphics and specify which 'density' function to use
plt1 = AlgebraOfGraphics.data(chain) * mapping(:iteration) * chain_mapping * visual(Lines)
plt2 = AlgebraOfGraphics.data(chain) * chain_mapping * AlgebraOfGraphics.density()  # Specified package for density

# Create and draw the figures
